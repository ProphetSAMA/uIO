...
//         // 从数据库获取所有房间的参数
//         List<RoomConfig> roomConfigs = powerMapper.getAllRoomConfigs();

//         // 遍历房间配置并调用 PowerService 的方法
//         for (RoomConfig config : roomConfigs) {
//             int buildingId = config.getBuildingId(); // 楼号
//             int floorId = config.getFloorId();       // 层号
//             int roomId = config.getRoomId();         // 房间号

//             // 构建房间号
//             String roomVerify = String.format("%d-%d--%d-%02d", buildingId, floorId, roomId / 100, roomId % 100);
            
//             // 调用 PowerService 的方法，传入参数
//             powerService.insertPowerValue(buildingId, floorId, roomId);
//             logger.info("定时任务执行了，房间号: {}", roomVerify);
//         }
//     }
